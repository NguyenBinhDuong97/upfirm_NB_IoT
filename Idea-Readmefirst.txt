...
Phase 5: Thêm phần mqqt wirless để truyền tín hiệu qua NB_IoT
	   ...
	   * Ý tưởng hiện nay:
       ...
       ++ Quá trình thực hiện như sau:
		  ...
		  để lần sau sẽ chỉ gửi data và ack (HostClient có thực sự cần biết điều này hay chỉ như một trạm phát 
		  trung gian?)
		++ Nạp:
		  + Máy tính gửi data kèm theo thông tin vị trí con trỏ được gắn ở đầu của chuỗi gửi xuống gọi là "STT+data"
		  + HostClient sẽ publish thông tin "STT+data"
		  + AppClient nhận được thông tin "STT+data" sẽ phân tích chuỗi STT riêng và data riêng
          + AppClient có STT chuyển đổi từ vị trí con trỏ thành số thứ tự word rồi + với base address để tính address cần để nạp
		  + AppClient chuyển đổi data thành uint32_t rồi nạp vào data cần nạp.
		  ...
		  + Máy tính gửi tín hiệu data xuống hostclient
		  + Tiếp tục
