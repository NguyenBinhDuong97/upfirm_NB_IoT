...
char DEFAULT_BYTE_STM32F4[2] = { 'F', 'F'};
/**
...
{
	for (uint8_t pos = 0; pos < ByteNumb; pos++)
	{
		*(destination + pos) = *(source + pos);
	}
	return 1;
...
{
	//uint8_t* advArray = new uint8_t[9];
	uint8_t advArray[9] = {0x00};
		...
		if (HexFile_Check_Record_Type(ui8_LineArray) == _DATA_)
		{
			...
			if (number != 0)
			{
				...
				else
				{
					...
					HexFile_Extract_Byte_From_A_Line(Array + (ui8_WordHexArray * 8), ui8_SubArray, (ui8_SurplusByte*2));
					for (uint8_t i = 0; i < (4 - ui8_SurplusByte)*2; i++)
					{
						*(ui8_SubArray + ui8_SurplusByte * 2 + i) = 'F';//do byte default cua stm32f4 la FF sau khi xoa chip se thay
					}
					HexFile_Convert_A_Word_LittleEndian(ui8_SubArray);
					HexFile_Write_Data_To_DataFile(ui8_SubArray, 4);
					Reset_Buffer(ui8_SubArray, 8);
