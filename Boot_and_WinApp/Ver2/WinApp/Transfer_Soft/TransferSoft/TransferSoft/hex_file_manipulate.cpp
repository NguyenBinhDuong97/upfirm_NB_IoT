		...
		if (HexFile_Check_Record_Type(ui8_LineArray) == _DATA_)
		{
			...
			if (number != 0)
			{
				...
				/* the function below was applied to test extract data proceess operate well or not
				 you can delete it when you feel dont need to use it */
				//HexFile_Write_Data_To_DataFile(Array, number);
				ui8_LengthHexArray = strlen((char*)Array);
				ui8_WordHexArray = ui8_LengthHexArray / 8;
				if (number % 4 == 0)
				{
					for (uint8_t i = 0; i < ui8_WordHexArray; ++i)
					{
						HexFile_Extract_WordByWord_From_A_Line(Array + (i * 8), ui8_SubArray);
						HexFile_Write_Data_To_DataFile(ui8_SubArray, 4);
				...
				{
					ui8_SurplusByte = number % 4;
				    for (int i = 0; i < ui8_WordHexArray; ++i)
				    {
					    HexFile_Extract_WordByWord_From_A_Line(Array + (i * 8), ui8_SubArray);
					    HexFile_Write_Data_To_DataFile(ui8_SubArray, 4);
					    Reset_Buffer(ui8_SubArray, 8);
				    }
					HexFile_Write_Data_To_DataFile( Array + (8*ui8_WordHexArray), ui8_SurplusByte);
				}
				Reset_Buffer(Array, NumberElements);
			}
		}
...
uint8_t HexFile_Calculate_Word_In_WordDataFile(void)
{
	...
	uint8_t ui8_LengthArray = 0;
	while (!SourceFile.eof())
	{
		Reset_Buffer(Array, 10);
		SourceFile.getline((char*)Array, 10);
		ui8_LengthArray = strlen((char*)Array);
		if (ui8_LengthArray == 8)
			ui32_DataFileWordSize++;
		else
			ui8_SurplusByte = ui8_SurplusByte + ui8_LengthArray/2;
	}
...
uint8_t HexFile_Send_WordDataFile(void)
{
	...
	while (!SourceFile.eof())
	{
		Reset_Buffer(ui8_SubArray, 10); // 10 la so phan tu co trong mang cua ui8_LineArray
		SourceFile.getline((char*)ui8_SubArray, 10);
		//HexFile_Convert_A_Word_LittleEndian(ui8_SubArray);
		ui8_LengthArray = strlen((char*)ui8_SubArray);
		SeriApp_COM_SendArray((char*)ui8_SubArray, ui8_LengthArray);
